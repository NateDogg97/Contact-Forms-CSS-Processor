...
/**
 * FieldListItemDraggable - A draggable version of the field list item
 * 
 ...
 */
export default function FieldListItemDraggable({ 
  field, 
  index, 
  onEdit, 
  onDelete 
}) {
  // Helper to check if a field has options
  ...
  // Helper to format options display
  ...
  return html`
    <div 
      class="grid grid-cols-5 items-center space-x-4 p-4 border rounded bg-white hover:bg-gray-50 transition-colors relative"
      key=${field.id} 
    >
      <!-- Drag Handle -->
      <div class="absolute left-2 top-0 bottom-0 flex items-center cursor-grab drag-handle">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 9h16.5m-16.5 6.75h16.5" />
        </svg>
      </div>
      
      <!-- Field Content (shifted right to make room for handle) -->
      <div class="grid col-span-4 grid-cols-4 gap-4 pl-5">
        <div>
        ...
        ${hasOptions(field) && field.options && html`
